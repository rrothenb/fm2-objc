...
struct Vec {        // Usage: time ./explicit 16 && xv image.ppm
  ...
  Vec operator+(thread const Vec &b) const { return Vec(x+b.x,y+b.y,z+b.z); }
  Vec operator-(thread const Vec &b) const { return Vec(x-b.x,y-b.y,z-b.z); }
  Vec operator*(float b) const { return Vec(x*b,y*b,z*b); }
  Vec mult(thread const Vec &b) const { return Vec(x*b.x,y*b.y,z*b.z); }
  thread Vec& norm(){ return *this = *this * (1/sqrt(x*x+y*y+z*z)); }
  float dot(thread const Vec &b) const { return x*b.x+y*b.y+z*b.z; } // cross:
  Vec operator%(thread Vec&b){return Vec(y*b.z-z*b.y,z*b.x-x*b.z,x*b.y-y*b.x);}
};
...
Vec radiance(thread const Ray &r, int depth, thread float *Xi,int E=1){
  ...
  if (obj.refl == DIFF){                  // Ideal DIFFUSE reflection
    float r1=2*M_PI*random(Xi), r2=random(Xi), r2s=sqrt(r2);
    ...
    for (int i=0; i<numSpheres; i++){
      ...
      if (intersect(Ray(x,l), t, id) && id==i){  // shadow ray
        ...
        e = e + f.mult(s.e*l.dot(nl)*omega)*M_1_PI_F;  // 1/pi for brdf
      }
    }
